...
uint8_t put_in_string(uint16_t number, char letter, uint8_t place)//place is 4 digit spotplus 2 for something else, count from right
{
	//
	//'place' is the target place for the next character 
	//
	//
	char temps[6] = "\0\0\0\0\0\0";//temporary spot for the number to add to the big string 

	if (number <= 65535)
	{
		//ultoa
		utoa(number, temps, 10); // integer number --> string temps base 10
	}
	else
	{
		strcpy(temps,"over");// 
