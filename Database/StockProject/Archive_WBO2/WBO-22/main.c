...
volatile uint16_t nurnst_data[NURNSTPOINTS+1];

//for slope determination of nurnst:
#define IPPOINTS 3 //number of points to track
volatile uint16_t Ip_data[IPPOINTS+1];
...
volatile int16_t nurnst_slope = 0;
volatile int16_t ip_slope = 0;
...
void data_nurnst_update(uint16_t newval);		
void data_nurnst_backup(void);		
uint16_t data_nurnst_extrap(void);		
void data_ip_update(uint16_t newval);		
void data_ip_backup(void);		
uint16_t data_ip_extrap(void);
...
//	charspot = put_in_string(DC_val,'d',charspot);//puts data in big string
	...
	data_ip_update(pump);
	data_nurnst_update(nurnst);
	
	uint16_t nurnst_calc = 0;
	uint16_t ip_calc = 0;

	if( nurnst_spike == 0)// nurnst was not spiking
	{
		if( abs((nurnst_data[NURNSTPOINTS-2])-nurnst) > 30)//nurnst spiked 35 or more in one cycle
		{
			nurnst_spike = 1;
			//backup
			data_ip_backup();
			data_nurnst_backup();
			//create extrapolated values for nurnst and ip 
			nurnst_calc = data_nurnst_extrap();
			ip_calc = data_ip_extrap();
		}
		else{nurnst_calc = nurnst; ip_calc = pump;}//no spike, just pass values on.
	}
	else//nurnst is in a spike
	{
		timeinspike++;
		data_nurnst_backup();
		nurnst_calc = data_nurnst_extrap();
		if ( (abs(nurnst_calc-nurnst)<10)||(timeinspike>10) )//back in range or not a spike
		{
			nurnst_calc = nurnst; ip_calc = pump;
			data_nurnst_update(nurnst);//restor measured values
			nurnst_spike = 0;
			timeinspike = 0;
			nurnst_calc = nurnst; ip_calc = pump;//pass values on
		}
		else //or not in range, continue to use extrapolated
		{
			data_ip_backup();
			ip_calc = data_ip_extrap();
		}
	...
	//if no spiked flag:
		//check for nurnst spike (check nurnst_data and current value.  >35)
		//if spiked, 
			//calculate replacement nurnst value based on slope so far nurnst_calc(previous average plus slope.)
			//calculate replacement Ip same way.
			//set spiked flag.
		//if no spike , nurnst_calc = nurnst, ip_calc = pump
	//if spiked flag:
		//calculate nurnst_calc and ip_calc:   
		// If  nurnst_calk is with 10 of nurnst
			//go back to using real values. nurnst_calc = nurnst, ip_calc = pump
			//spiked flag 0
		//Nurnst still not back,
			//just keep using calculated values.
	//submit nurnst_calc and ip_calc to PID and slope.
	////////////count loops in spike: if too many, just go out of spike.n 
...
void data_nurnst_update(uint16_t newval)		
{
	for (uint8_t i = 0; i<(NURNSTPOINTS); i++)//shift points
	{
		nurnst_data[i]=nurnst_data[i+1]; 
	}
	nurnst_data[NURNSTPOINTS] = newval;//put in latest value
}
void data_nurnst_backup(void)		
{
	for (uint8_t i = (NURNSTPOINTS); i>0; i--)//shift points backwards
	{
		nurnst_data[i]=nurnst_data[i-1];
	}
}
uint16_t data_nurnst_extrap(void)//extrapolates 1 point		
{//calculate sums for least squares:
	...
	int16_t SUMxx  = 0;
	uint16_t average = 0;
	for (uint8_t i = 1; i<(NURNSTPOINTS+1); i++)//SUMx
	{
		SUMx = SUMx+i;
		SUMy = SUMy+nurnst_data[i];
		SUMxy = SUMxy+(i*nurnst_data[i]);
		SUMxx = SUMxx+(i*i);
		average = average+nurnst_data[i];
		
	}
	int16_t slope = ((SUMx*SUMy)- NURNSTPOINTS*SUMxy) / ( (SUMx*SUMx) - NURNSTPOINTS*SUMxx);
	average = average/NURNSTPOINTS;// average
//	uint16_t rise = ((NURNSTPOINTS+2)*(abs(slope)))/2;
//	uint16_t newpoint = 0;
//	if (slope<0){newpoint = average-rise;}
//	else {newpoint = average+rise;}
...
void data_ip_update(uint16_t newval)		
{
	for (uint8_t i = 0; i<(IPPOINTS); i++)//shift points
	{
		Ip_data[i]=Ip_data[i+1];
	}
	Ip_data[IPPOINTS] = newval;//put in latest value
}
void data_ip_backup(void)		
{
	for (uint8_t i = (IPPOINTS); i>0; i--)//shift points backwards
	{
		Ip_data[i]=Ip_data[i-1];
	}
}
uint16_t data_ip_extrap(void)//extrapolates 1 point		
{
	//calculate sums for least squares:
	int8_t SUMx = 0; 
	int16_t SUMy =0;
	...
	int16_t SUMxx  = 0;
	uint16_t average = 0;
	for (uint8_t i = 1; i<(IPPOINTS+1); i++)//SUMx
	{
		SUMx = SUMx+i;
		SUMy = SUMy+Ip_data[i];
		SUMxy = SUMxy+(i*Ip_data[i]);
		SUMxx = SUMxx+(i*i);
		average = average+Ip_data[i];
	}
	int16_t slope = ((SUMx*SUMy)- IPPOINTS*SUMxy) / ( (SUMx*SUMx) - IPPOINTS*SUMxx);
	average = average/IPPOINTS;// average
//	uint16_t rise = ((IPPOINTS+2)*(abs(slope)))/2;
//	uint16_t newpoint = 0;
//	if (slope<0){newpoint = average-rise;}
//	else {newpoint = average+rise;}
