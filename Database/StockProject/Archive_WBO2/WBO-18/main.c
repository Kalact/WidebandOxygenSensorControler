...
#define NUM_BOXES 10//for boxcar averaging.  Set to 1 for no averaging
...
void PID_pump(uint16_t nurnst_val)
{
	...
	calculated = ipramp;
//calculated = (calculated) + 116;//116 is zero current.
...
uint8_t nurnst_slope_tracker(uint16_t nurnstval)		
{
	...
	//charspot = put_in_string(abs(slope),'\0',charspot);//puts data in big string
	return abs(slope);
