...
//	charspot = put_in_string(DC_val,'d',charspot);//puts data in big string
	...
	//calculate 0-5v output from Look Up Table:
	struct two_col{
		uint16_t x;
		uint32_t y;
	}; 
	struct two_col out_volts[]={	//table columns: pump, zero_to_5_WB (pump current ADC value, DAC value)
		{400,0},
		{425, 10}, 
		{450, 20},
		{475, 30},
	...
	//out of range check:
	if (pump<out_volts[0].x)//smaller than the lowest value in LOT
	{
		zero_to_5_WB = 0;
	}
	else if (pump>out_volts[n-1].x)//larger than the highest value in the LOT
	{
		zero_to_5_WB = 254;
	}
	else//in range, lookup in table, interpolate
	{
		
		for( uint8_t i = 0; i < n-1; i++ )//loop through table and find value
		{
			if ( out_volts[i].x <= pump && out_volts[i+1].x >= pump )
			{
				uint16_t diffx = pump - out_volts[i].x; //difference between the pump value and the x value in the LOT
				uint16_t diffn = out_volts[i+1].x - out_volts[i].x;//spacing between the values in the LOT
				zero_to_5_WB = out_volts[i].y + ( out_volts[i+1].y - out_volts[i].y ) * diffx / diffn; //output value is interpolated.
			}
		}
	}
