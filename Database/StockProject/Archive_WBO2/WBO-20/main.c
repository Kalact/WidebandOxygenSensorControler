...
#define NURNSTPOINTS 10 //number of points to track
uint32_t nurnst_data[NURNSTPOINTS];
...
volatile uint8_t nspike = 0;//flag for nurnst spike detected
volatile uint8_t near_stoic_flag = 0;
...
uint8_t nurnst_slope_tracker(uint16_t nurnstval);
uint16_t boxcaravg(uint16_t average, uint8_t no_new);
...
void seven(void)//measure nurnst 
{
	...
	uint8_t slope = nurnst_slope_tracker(nurnst);
	charspot = put_in_string(slope,'\0',charspot);//puts data in big string
	//uint16_t fixednurnst = 0;
	//if (slope>0){fixednurnst = TARGET_NERNST;}
	//else {fixednurnst = nurnst;}
	//charspot = put_in_string(fixednurnst,'\0',charspot);//puts data in big string
...
//	charspot = put_in_string(DC_val,'d',charspot);//puts data in big string
	...
	pump = boxcaravg(pump,0);//average pump current
...
void PID_pump(uint16_t nurnst_val)
{
	...
	calculated = (calculated) + ZERO_CURRENT;// is zero current.
	
	if ( (calculated < (ZERO_CURRENT+15+25))&&(calculated >(ZERO_CURRENT+15-25) ))//in near stoic range 
	{
		if((!near_stoic_flag)&&(calculated > (ZERO_CURRENT+25)))//ramp ip down slowly, no PID
		{
			ipramp = ZERO_CURRENT+25+10;
			near_stoic_flag = 1;//1=going down, 2 = going up
		}
		if((!near_stoic_flag)&&(calculated <= (ZERO_CURRENT+25)))//ramp ip up slowly, no PID
		{
			ipramp = ZERO_CURRENT+25-10;
			near_stoic_flag = 2;//1=going down, 2 = going up
		}
		#define RAMP_SLOPE 4//number of loops before incrementing/decrimenting pump power
		else //already in near stoic range
		{
			IpCount ++;
			if (IpCount>100){IpCount = RAMP_SLOPE+1;}
		}
		if (IpCount == RAMP_SLOPE)// 
		{
			if (!nspike)
			{	
				if (near_stoic_flag == 2)
				{
					ipramp++;
				}
			...
			{
				IpCount = RAMP_SLOPE+1; //just keep looping until calculated is out of range
			}
		}
		calculated = ipramp;
	}
...
uint16_t boxcaravg(uint16_t average, uint8_t no_new)
{
	if (no_new ==0)// shift inthe new value, unless no_new = 1, just return the past average
	{
		for (uint8_t i = 0; i<(NUM_BOXES-1); i++)//shift boxcars
		{
			box_data[i]=box_data[i+1];
		}
		box_data[NUM_BOXES-1] = average;//put in latest value
	}
	...
	return(avrg_data_tot/NUM_BOXES);//get averaged value
}