...
volatile uint8_t use_next = 0;//flag for nurnst spike =0, but was >0
...
void seven(void)//measure nurnst 
{
	...
	//maybe only if punm V is close to ZERO_CURRENT?
	if (slope>0)//quick change of nurnst
	{
		nspike = 1;//set flag
		//use average of last 3 Ip for Ip
	}
	else//no quick change of nurnst
	{
		if (nspike == 1)//already set, then average in next Ip point
		{
			use_next = 1;//set flag for 'use next point in ave'
		} 
		nspike = 0;
	}
...
//	charspot = put_in_string(DC_val,'d',charspot);//puts data in big string
	...
	if (nspike)
	{
		if (use_next == 2)
		{
			pump = boxcaravg(pump,0);//average in this point, and use average
			use_next = 0;
		}
		else 
		{
			pump = boxcaravg(0,1);//don't average in this point, use past average
		}
	}
		
	else //no nurnst spike
	{
		if (use_next == 1)
		{
			pump = boxcaravg(0,1);//don't average in this point, use average
			use_next = 2;
		}
		else
		{
...
//	IpCount ++;
//	if (IpCount > 40UL)//number of loops before incrementing Ip power 
//	{
//		ipramp++;
//		IpCount = 0;//reset counter
//		if (ipramp>MAXPUMP_I) {ipramp = MINPUMP_I;}//reset to 
//		if (ipramp<MINPUMP_I) {ipramp = MINPUMP_I;}//start at 
//	}
//	calculated = ipramp;
