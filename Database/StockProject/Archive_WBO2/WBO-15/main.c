...
/////////////---------------------------------/////////////////
/*
 * has pump ramp, not PID.  In PID function.
 */
...
#define MAXPUMP_I 254 // maximum current value for pump (leanest).
...
void PID_pump(void)
{
	...
	IpCount ++;
	if (IpCount > 80UL)//nmber of loops before incrementing Ip power 
	{
		ipramp++;
		IpCount = 0;//reset counter
		if (ipramp>MAXPUMP_I) {ipramp = 180UL;}//reset to 180
		if (ipramp<MAXPUMP_I-100) {ipramp = 180UL;}//start at 180
	}
	calculated = ipramp;
